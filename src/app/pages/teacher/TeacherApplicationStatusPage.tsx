  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!mounted) return;
        setApp(apps.find((a) => a.teacherId === user.id) ?? null);
      } catch (err) {
        // ignore
      }
    })();
        ...
        <div className="max-w-md mx-auto p-4">
          <div className="bg-white rounded-2xl shadow-lg p-6 text-center">
            ...
            <p className="text-sm text-gray-600 mt-2">
              قم بتقديم طلبك ورفع المستندات المطلوبة.
            </p>
            <button
              onClick={() => navigate("/teacher/apply")}
        ...
        <div className="bg-white rounded-2xl shadow-lg p-5 sm:p-8">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="text-sm text-gray-600 mt-1 break-words">
                {app.teacherName} • {app.email}
              </p>
            </div>
            <div
              className={`px-3 py-2 rounded-xl text-sm font-bold ${statusChip.cls} flex items-center gap-2 flex-shrink-0`}
            >
          ...
          <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="p-4 rounded-2xl border border-gray-200">
              ...
              <p className="font-bold text-gray-900">
                {app.reviewedAt
                  ? new Date(app.reviewedAt).toLocaleDateString("ar-SA")
                  : "لم يتم بعد"}
              </p>
            </div>
          </div>
          ...
          {app.status === "approved" && (
            <div className="mt-4 p-4 rounded-2xl bg-emerald-50 border border-emerald-200">
              ...
              <p className="text-sm text-emerald-700 mt-1">
                يمكنك الآن إعداد خدماتك وتسعيرك.
              </p>
              <button
